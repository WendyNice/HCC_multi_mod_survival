...
def roi_gen(path_save_png, name, img, roi, shape, radius):
    ...
    # for idx in idx_all:
    #     img_seg_2 = roi[idx, :, :]
    #     dilated_seg = multi_dilation(img_seg_2, 2)
    ...
    img_new[:int(radius / 2), :, :] = img_roi[int(slice_num - radius / 4):int(slice_num + radius / 4),
    ...
    plt.close()
    # plt.show()
    plt.imshow(img_new[10, :, :], cmap='gray')
    plt.title(path_img)
...
for i in pat:
    ...
    # AP_path = os.path.join(img_path, 'AP', 'AP_bias.nii.gz')
    # PVP_path = os.path.join(img_path, 'PVP', 'PVP_bias.nii.gz')
    # T1WI_path = os.path.join(img_path, 'T1WI', 'T1WI_bias.nii.gz')
    # T2WI_path = os.path.join(img_path, 'T2WI', 'T2WI_bias.nii.gz')

    # AP_path = os.path.join(img_path, 'AP', 'AP_registered.nii.gz')
    # PVP_path = os.path.join(img_path, 'PVP', 'PVP_registered.nii.gz')
    # T1WI_path = os.path.join(img_path, 'T1WI', 'T1WI_registered.nii.gz')
    # T2WI_path = os.path.join(img_path, 'T2WI', 'T2WI_registered.nii.gz')
    ...
    if os.path.exists(AP_mask_path) and os.path.exists(PVP_mask_path) and os.path.exists(T1WI_mask_path):
        ...
        AP_new = roi_gen(path_save_png, i+'AP', AP_img, AP_mask, (20, 80, 80), radius)
        PVP_new = roi_gen(path_save_png, i+'PVP', PVP_img, PVP_mask, (20, 80, 80), radius)
        T1WI_new = roi_gen(path_save_png, i+'T1WI', T1WI_img, T1WI_mask, (20, 80, 80), radius)

        img_new = np.zeros((3, 20, 80, 80), dtype=AP_new.dtype)

        img_new[0, :, :, :] = AP_new
        img_new[1, :, :, :] = PVP_new
        img_new[2, :, :, :] = T1WI_new
        print('min, max img_new', np.max(img_new), np.min(img_new))
        plt.imshow(img_new[0, :, :, 40], cmap='gray')
        plt.title(i)
