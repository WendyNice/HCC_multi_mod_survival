...
data_save = '/data/Wendy/HCC/MR_clf/data_OS_resample'
label_save = '/data/Wendy/HCC/MR_clf/label_OS_resample'
print('time = 1826')
time_set = 1826
event_type = '死亡时间'
#############################################################
...
for i in range(len(data['event_time'].tolist())):
    if not math.isnan(data['event_time'].tolist()[i]):
        ...
        if data['event_time'].tolist()[i] >= time_set-60:
        ...
        if data['follow_time'].tolist()[i] >= time_set-60:
            label_event.append(0)
            time_event.append(time_set)
        else:
...
for i in range(len(train_ID)):
    if str(int(train_ID[i])) not in del_pat or str(int(train_ID[i])) not in data_roi_not_match:
        ...
        if str(int(train_ID[i])) + '.nii.gz' in patient_ls:
            if (int(train_data_ori['label_event'].tolist()[i]) < 2) or \
                    (int(train_data_ori['time_event'].tolist()[i]) > 1400):
                # print('copy')
                ...
                print(train_data_ori['time_event'].tolist()[i])
                print('label == 2')
...
for i in range(len(test_ID)):
    ...
    if str(int(test_ID[i])) not in del_pat or str(int(test_ID[i])) not in data_roi_not_match:
        ...
        if str(int(test_ID[i])) + '.nii.gz' in patient_ls:
            if int(test_data_ori['label_event'].tolist()[i]) < 2 or \
                    (int(test_data_ori['time_event'].tolist()[i]) > 1400):
                # print('copy')
                ...
                print(test_data_ori['time_event'].tolist()[i])
                print('label == 2')

np.save(os.path.join(label_save, 'test_id'), test_in)
np.save(os.path.join(label_save, 'train_id'), train_in)
print('train_in', len(train_in), len(test_in))
np.save(os.path.join(label_save, 'train_label'), train_label)
np.save(os.path.join(label_save, 'test_label'), test_label)
