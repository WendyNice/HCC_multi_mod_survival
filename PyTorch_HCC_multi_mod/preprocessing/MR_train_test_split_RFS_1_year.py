...
path_data = '/data/Wendy/HCC/ROI_NII_resample'
data_save = '/data/Wendy/HCC/MR_clf/data_RFS_resample_1_year'
label_save = '/data/Wendy/HCC/MR_clf/label_RFS_resample_1_year'
print('time = 365')
time_set = 365
event_type = '复发时间'
#############################################################
...
for i in range(len(data['event_time'].tolist())):
    print('data[].tolist()[i]', data['event_time'].tolist()[i])
    if (not math.isnan(data['event_time'].tolist()[i])):
            # and (data['event_time'].tolist()[i] > 90):
...
if not os.path.exists(os.path.join(data_save, 'train')):
    os.makedirs(os.path.join(data_save, 'train'))
if not os.path.exists(os.path.join(data_save, 'test')):
    os.makedirs(os.path.join(data_save, 'test'))
...
for i in range(len(train_ID)):
    if str(int(train_ID[i])) not in del_pat or str(int(train_ID[i])) not in data_roi_not_match:
        ...
        if str(int(train_ID[i])) + '.nii.gz' in patient_ls:
            if (int(train_data_ori['label_event'].tolist()[i]) < 2) and (int(train_data_ori['time_event'].tolist()[i]) > 90):
                # print('copy')
                train_in.append(train_ID[i])
                train_label.append(train_data_ori['label_event'].tolist()[i])
                shutil.copy(os.path.join(path_data, str(int(train_ID[i]))+'.nii.gz'), os.path.join(data_save, 'train'))
            else:
                print('label == 2')
    # else:
    #     if str(int(train_ID[i])) + '.nii.gz' in patient_ls:
    #         train_in.append(train_ID[i])
    #         train_label.append(train_data_ori['label_event'].tolist()[i])
    #         img = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(path_data, str(int(train_ID[i])) + '.nii.gz')))
    #         print(str(i) + 'not in', img.shape)
...
for i in range(len(test_ID)):
    ...
    if str(int(test_ID[i])) not in del_pat or str(int(test_ID[i])) not in data_roi_not_match:
        ...
        if str(int(test_ID[i])) + '.nii.gz' in patient_ls:
            if (int(test_data_ori['label_event'].tolist()[i]) < 2) and (int(test_data_ori['time_event'].tolist()[i]) > 90):
                # print('copy')
                test_in.append(test_ID[i])
                test_label.append(test_data_ori['label_event'].tolist()[i])
                shutil.copy(os.path.join(path_data, str(int(test_ID[i]))+'.nii.gz'), os.path.join(data_save, 'test'))
            else:
                print('label == 2')
    # else:
    #     if str(int(train_ID[i])) + '.nii.gz' in patient_ls:
...
np.save(os.path.join(label_save, 'test_id_1_year'), test_in)
np.save(os.path.join(label_save, 'train_id_1_year'), train_in)
np.save(os.path.join(label_save, 'train_label_1_year'), train_label)
np.save(os.path.join(label_save, 'test_label_1_year'), test_label)
print(len(train_in))
