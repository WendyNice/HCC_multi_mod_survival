...
time_set = 1826
# time_set = 730
print('死亡时间', label_df['死亡时间'].tolist())
print('手术时间', label_df['手术时间'].tolist())
label_df['event_time'] = [i.days for i in label_df['死亡时间'] - label_df['手术时间']]
...
test_csv.to_csv(os.path.join(csv_path, 'val_fold3mod_5_survival.csv'), header=False, index=False, sep=str(','))
train_csv.to_csv(os.path.join(csv_path, 'train_fold3mod_5_survival.csv'), header=False, index=False, sep=str(','))
