...
def roi_gen(path_save_png, name, img, roi, shape, radius):
    ...
    plt.close()
    # plt.show()
    plt.imshow(np.concatenate((img_new[40, :, :], img_new[:, 40, :], img_new[:, :, 40]), axis=1), cmap='gray')
    plt.title(path_img)
...
path_save = '/data/Wendy/HCC/ROI_NII_resample'
path_save_png = '/data/Wendy/HCC/ROI_png_resample'
...
for i in pat:
    ...
    AP_mask_path = os.path.join(path_mask, 'AP_mask_' + i + '.nii.gz')
    PVP_mask_path = os.path.join(path_mask, 'PVP_mask_' + i + '.nii.gz')
    T1WI_mask_path = os.path.join(path_mask, 'T1WI_mask_' + i + '.nii.gz')
